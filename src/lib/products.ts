 ...
 * 30 products across 9 populated categories (water_softener is defined but empty:
 * softener units are sold through installers, so that segment is served by the
 * quote form; softener_salt carries the Amazon consumables for the cluster).
 *
...
export function estimatedEarningsGbp(product: FilterProduct): number | null {
  ...
  // A percentage of an unknown price (priceGbp 0 renders "Check price") is a
  // guess dressed as a number, so it gets the same blank as an unconfirmed rate.
